...
def get_stats():
    ...
    cur.execute('SELECT SUM(balance) FROM stars_balance')
    total_stars = cur.fetchone()[0] or 0
    conn.close()
    return users, orders, total, total_stars
...
def stars_amount_menu(target_user=None):
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("50 ⭐ — 76 ₽", callback_data=f"stars_50_{target_user or 'me'}")],
        [InlineKeyboardButton("100 ⭐ — 152 ₽", callback_data=f"stars_100_{target_user or 'me'}")],
        [InlineKeyboardButton("250 ⭐ — 380 ₽", callback_data=f"stars_250_{target_user or 'me'}")],
        [InlineKeyboardButton("500 ⭐ — 760 ₽", callback_data=f"stars_500_{target_user or 'me'}")],
        [InlineKeyboardButton("1000 ⭐ — 1 520 ₽", callback_data=f"stars_1000_{target_user or 'me'}")],
        [InlineKeyboardButton("5000 ⭐ — 7 600 ₽", callback_data=f"stars_5000_{target_user or 'me'}")],
...
def premium_menu():
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("3 мес. — 989 ₽ (651 ★)", callback_data="premium_3")],
        [InlineKeyboardButton("6 мес. — 1 219 ₽ (802 ★)", callback_data="premium_6")],
        [InlineKeyboardButton("12 мес. — 2 130 ₽ (1 401 ★)", callback_data="premium_12")],
...
def steam_menu():
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("5$ — 350 ₽ (230 ★)", callback_data="steam_5")],
        [InlineKeyboardButton("10$ — 700 ₽ (460 ★)", callback_data="steam_10")],
        [InlineKeyboardButton("25$ — 1 750 ₽ (1 150 ★)", callback_data="steam_25")],
        [InlineKeyboardButton("50$ — 3 500 ₽ (2 300 ★)", callback_data="steam_50")],
        [InlineKeyboardButton("100$ — 7 000 ₽ (4 600 ★)", callback_data="steam_100")],
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("pay_"):
                ...
                async def check_payment():
                    try:
                        ...
                        if final_order.status == "delivered":
                            add_stars_balance(user_id, int(amount))
                            await context.bot.send_message(
                                user_id,
                                format_message(
                                    "✅ Оплата подтверждена!",
                                    f"{amount} ★ зачислены @{recipient}!"
                                ),
                                parse_mode="Markdown"
                            )
        ...
        if method == "btc":
            ...
            text += f"Заказ: {order_type} на {amount}\n"
            text += f"Сумма: {int(amount) * 0.0005} BTC\n\n"
            text += f"📍 **Адрес:**\n`{WALLETS['btc']}`\n\n"
            text += "❗ Счёт действителен 30 минут.\n"
            text += "После оплаты пришлите скриншот чека в этот чат."
            await query.edit_message_text(
        ...
        if method == "eth":
            ...
            text += f"Заказ: {order_type} на {amount}\n"
            text += f"Сумма: {int(amount) * 0.008} ETH\n\n"
            text += f"📍 **Адрес:**\n`{WALLETS['eth']}`\n\n"
            text += "❗ Счёт действителен 30 минут.\n"
            text += "После оплаты пришлите скриншот чека в этот чат."
            await query.edit_message_text(
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    users, orders, total_ton, total_stars = get_stats()
    
    text = f"📊 **Статистика**\n\n"
    text += f"👥 Всего пользователей: {users}\n"
    text += f"📦 Всего заказов: {orders}\n"
    text += f"💰 Общая сумма: {total_ton} TON\n"
    text += f"⭐ Всего звёзд: {total_stars}\n"
